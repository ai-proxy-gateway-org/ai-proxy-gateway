// Sahte sağlayıcı — OpenAI, Anthropic ve Gemini'nin yerine geçer.
//
// Eskiden yalnızca ayrı bir süreç olarak (localhost:4000) çalışıyordu. Bu,
// canlıda kullanılamaz demekti: Vercel'in sunucusu bizim makinemizdeki mock'a
// erişemez, dolayısıyla dağıtılmış sistemde hiç istek atılamazdı.
//
// Artık bir eklenti: hem tek başına çalışabiliyor (npm run mock) hem de ana
// uygulamanın içine `/mock` yolu altına takılabiliyor. Böylece gerçek sağlayıcı
// anahtarları olmadan da canlıda uçtan uca gösterim yapılabiliyor.
//
// Ana uygulamada yalnızca ENABLE_MOCK_PROVIDERS=true iken açılıyor; bayrak
// olmadan bu yollar hiç tanımlanmıyor.

import type { FastifyInstance, FastifyReply } from 'fastify';

export async function mockProviderRoutes(server: FastifyInstance) {
...
server.addHook('preHandler', async (request) => {
  // Terminale basılan kutu sunum içindi; sunucusuz ortamda kimse görmüyor.
  if (process.env.MOCK_STANDALONE !== 'true') return;
...
server.post('/v1beta/models/:target', async (request, reply) => {
